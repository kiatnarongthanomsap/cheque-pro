...
import { Printer, Settings, RefreshCcw, Check, Move, MousePointer2, RotateCcw, Languages, FileText, Download, Trash2, History, Building2, User, Loader2, Save, LogOut, LogIn, Landmark, Calendar, Hash, UserCheck, Banknote, Ban, Undo2, AlertTriangle, Type, Database, Facebook, Mail, Smartphone } from 'lucide-react';
import { initiateLineLogin } from './services/lineAuth';
...
// --- Main Component ---
export default function ChequePrinter({ lineUserInfo }) {
    // --- States ---
    ...
    const handleGoogleSignIn = useCallback((response) => {
        ...
        try {
            ...
            const userId = `u_google_${payload.sub}`;
            const displayName = payload.name || payload.email?.split('@')[0] || 'Google User';
            const email = payload.email || `google_${Date.now()}@google.id`;
            ...
            alert('เกิดข้อผิดพลาดในการประมวลผลข้อมูลจาก Google');
    ...
    useEffect(() => {
        ...
        // หมายเหตุ: ต้องตั้งค่า Client ID ใน Google Cloud Console ก่อนใช้งาน
        // ดูคำแนะนำในไฟล์ GOOGLE_SIGNIN_SETUP.md
        if (window.google?.accounts?.id && !user) {
            try {
                window.google.accounts.id.initialize({
                    client_id: 'YOUR_GOOGLE_CLIENT_ID', // ⚠️ ต้องเปลี่ยนเป็น Client ID จาก Google Cloud Console
    ...
    // --- Handle Line Login Result ---
    useEffect(() => {
        if (lineUserInfo) {
            performLogin(lineUserInfo);
        }
    }, [lineUserInfo]);
        ...
        if (provider === 'google') {
            ...
            try {
                if (!window.google?.accounts?.id) {
                    alert('Google Sign-In ยังไม่พร้อมใช้งาน กรุณาตรวจสอบการตั้งค่า');
                    return;
                }

                setAuthLoading(true);
                
                // Trigger Google Sign-In popup using One Tap or button flow
                window.google.accounts.oauth2.initTokenClient({
                    ...
                    callback: (tokenResponse) => {
                        if (tokenResponse.access_token) {
                            ...
                            .then(res => res.json())
                            .then(userInfo => {
                                const userId = `u_google_${userInfo.id}`;
                                const displayName = userInfo.name || userInfo.email?.split('@')[0] || 'Google User';
                                
                                performLogin({
                                    id: userId,
                                    provider: 'google',
                                    displayName: displayName,
                                    email: userInfo.email || `google_${Date.now()}@google.id`,
                            ...
                            .catch(error => {
                                ...
                                alert('เกิดข้อผิดพลาดในการดึงข้อมูลผู้ใช้จาก Google');
                                setAuthLoading(false);
                            });
                        } else {
                            setAuthLoading(false);
                ...
                if (error.error !== 'popup_closed_by_user' && error.error !== 'access_denied') {
                    alert('เกิดข้อผิดพลาดในการเข้าสู่ระบบด้วย Google: ' + (error.error || error.message || 'Unknown error'));
                }
                setAuthLoading(false);
            }
            ...
            // Simulate other Social IDs (Facebook)
            const names = {
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-6">
                        <FacebookLogin
                            ...
                            onLoginSuccess={(userData) => {
                                performLogin({
                                    id: `u_facebook_${userData.id}`,
                                    provider: 'facebook',
                                    displayName: userData.name,
                                    email: userData.email || `facebook_${userData.id}@facebook.id`,
                        ...
                        <button 
                            ...
                            className="flex flex-col items-center justify-center gap-1 p-3 rounded-lg border border-gray-200 hover:bg-red-50 hover:border-red-200 transition group disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            {authLoading ? (
